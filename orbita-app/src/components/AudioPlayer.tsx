...
const AudioPlayer: React.FC<AudioPlayerProps> = ({ currentEpisode }) => {
  ...
  React.useEffect(() => {
    ...
    const handleEnded = () => {
      ...
      if (currentEpisode && !listenedEpisodes.includes(String(currentEpisode.id))) { 
        localStorage.setItem('listenedEpisodes', JSON.stringify([...listenedEpisodes, String(currentEpisode.id)]));
      }
    };
