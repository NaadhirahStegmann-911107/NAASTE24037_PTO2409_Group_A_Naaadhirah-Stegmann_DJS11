...
import type { Show, Season, Episode, Favorite } from "./types";
...
interface ShowViewProps {
  ...
  favorites: Favorite[];
  toggleFavorite: (showId: number, seasonId: number, episodeId: number, showTitle: string, seasonTitle: string, episodeTitle: string) => void;
  setView: (view: 'home' | 'show') => void;
}

export const ShowView: React.FC<ShowViewProps> = ({
  show,
  selectedSeason,
  setSelectedSeason,
  currentEpisode,
  ...
  return (
    <div>
      <button onClick={() => setView('home')} className="back-btn" aria-label="Back to shows list">
        Back to Shows
      </button>
        ...
        <div>
          ...
          <ul className="episode-list">
            {selectedSeason.episodes.map((episode) => {
              ...
              return (
                <li key={episode.id} className={isCurrent ? 'current-episode' : ''}>
                  <p>{episode.title}</p>
                  <button
                    onClick={() => setCurrentEpisode(episode)}
                    className={`play ${isCurrent ? 'playing' : ''}`}
                    aria-label={isCurrent ? `Playing episode ${episode.title}` : `Play episode ${episode.title}`}
                    disabled={isCurrent}
                  >
                    {isCurrent ? 'Playing...' : 'Play'}
                  </button>
                  <button
                    onClick={() => toggleFavorite(show.id, selectedSeason.id, episode.id, show.title, selectedSeason.title, episode.title)}
